...
module mo_usrrxt
  ...
  integer :: h2o_ndx, so4_ndx, cb2_ndx, oc2_ndx, nit_ndx
...
!-----------------------------------------------------------------
...
#else
    real(r8), target  :: sfc_array(pcols,pver,5), dm_array(pcols,pver,5)
#endif
    ...
    if( usr_NO2_aer_ndx > 0 .or. usr_NO3_aer_ndx > 0 .or. usr_N2O5_aer_ndx > 0 .or. usr_HO2_aer_ndx > 0 ) then
       if( carma_do_hetchem ) then
         ...
         call surfarea( mmr, pmid, temp, pbuf, ltrop, ncol, sfc_array, dm_array, sad_total )
#else
...
!-------------------------------------------------------------------------
  ...
  !-------------------------------------------------------------------------
  subroutine surfarea( mmr, pmid, temp, pbuf, ltrop, ncol, sfc, dm_aer, sad_total )
