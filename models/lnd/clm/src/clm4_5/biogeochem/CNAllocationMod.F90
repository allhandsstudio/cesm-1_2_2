      ...
      do fc=1,num_soilc
         ...
         if (sum_nh4_demand(c,j)*dt < smin_nh4_vr(c,j)) then
            ...
            if (sum_nh4_demand(c,j) > 0.0_r8) then
               actual_immob_nh4_vr(c,j) = min((smin_nh4_vr(c,j)/dt)*(potential_immob_vr(c,j)*compet_decomp_nh4 &
                                           / sum_nh4_demand_scaled(c,j)), potential_immob_vr(c,j))
               smin_nh4_to_plant_vr(c,j) = min((smin_nh4_vr(c,j)/dt)*(col_plant_ndemand(c)*nuptake_prof(c,j) &
                                           *compet_plant_nh4 / sum_nh4_demand_scaled(c,j)), col_plant_ndemand(c) &
                                           *nuptake_prof(c,j))
         ...
         if (sum_no3_demand(c,j)*dt < smin_no3_vr(c,j)) then
            ...
            if (sum_no3_demand(c,j) > 0.0_r8) then
               ...
               smin_no3_to_plant_vr(c,j) = min((smin_no3_vr(c,j)/dt)*((col_plant_ndemand(c)*nuptake_prof(c,j)-&
