               ...
               if (conc_ch4_rel(c,j) < 0._r8) then
                  ...
                  if (deficit > 1.e-3_r8 * scale_factor_gasdiff) then
                     if (deficit > 1.e-2_r8) then
                        write(iulog,*)'Note: sink > source in ch4_tran, sources are changing quickly ' // &
                                      'relative to diffusion timestep, and/or diffusion is rapid.'
                        g = cgridcell(c)
                        write(iulog,*)'Latdeg,Londeg=',grc%latdeg(g),grc%londeg(g)
                        write(iulog,*)'This typically occurs when there is a larger than normal diffusive flux.'
                        write(iulog,*)'If this occurs frequently, consider reducing land model (or methane model)' // &
                                      ' timestep, or reducing the max. sink per timestep in the methane model.'
                     end if
